      ...
      <div className="flex gap-2 mb-2">
        ...
        {!open && (
          <button
            ...
            className={`group flex items-center gap-2 px-6 py-3 rounded-full shadow-xl bg-white/80 backdrop-blur-xl border border-white/60 text-sm font-bold text-[${THEME.colors.primary}] hover:bg-white hover:scale-105 transition-all duration-300 ring-1 ring-white/50`}
            onClick={() => setOpen(true)}
          >
            <div className={`p-1.5 rounded-full bg-gradient-to-r from-indigo-300 to-purple-300 text-white shadow-sm group-hover:rotate-12 transition-transform duration-300`}>
              <FiMessageSquare className="w-4 h-4" />
            </div>
            <span className={`bg-clip-text text-transparent bg-gradient-to-r from-indigo-300 to-purple-300`}>Messages</span>
          </button>
        )}
      </div>
            ...
            <div className="max-h-[450px] overflow-y-auto bg-gradient-to-b from-white/40 to-white/80 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
              {recentChats.map(chat => (
                <Link href="/profile/messages" key={chat.id} className="flex items-center justify-between px-5 py-4 hover:bg-white/50 transition-colors border-b border-white/40 group relative overflow-hidden">
                  <div className={`absolute inset-0 bg-gradient-to-r from-[${THEME.colors.primary}]/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
                  <div className="flex items-center gap-4 relative z-10">
                    <div className="relative">
                      <Image src={chat.avatar} alt={chat.name} width={42} height={42} className={`rounded-full object-cover ring-2 ring-white shadow-sm group-hover:ring-[${THEME.colors.primary}]/50 transition-all duration-300`} />
                      {chat.unreadCount && (
                        <span className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></span>
                      )}
                    </div>
                    <div>
                      <p className={`text-sm font-bold text-gray-800 group-hover:text-[${THEME.colors.primary}] transition-colors`}>{chat.name}</p>
                  ...
                  <div className="relative z-10 flex flex-col items-end gap-1">
                    ...
                    {chat.unreadCount ? (
                      <div className={`min-w-[1.25rem] h-5 px-1.5 bg-gradient-to-r from-indigo-300 to-purple-300 text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-sm`}>
                        {chat.unreadCount}
                      </div>
                    ) : null}
                  </div>
              ...
              <div className="p-4 bg-white border-t border-gray-100 sticky bottom-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                <Link 
                  href="/profile/messages" 
                  className={`flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-gradient-to-r from-indigo-300 to-purple-300 text-white font-bold text-sm hover:shadow-md hover:scale-[1.02] transition-all duration-300 shadow-sm group`}
                >
                  Open full messages 
